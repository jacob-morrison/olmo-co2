...
# group = "OLMo25"
name = "grpo_all_mix_p64_4_8k_olmo3-instruct-dpo-lbc-v2__1__1763365141"
...
for (num_gpus, run) in runs:
    history = run.history(keys=["batch/filtered_prompts", "val/sequence_lengths"])
    
    total_tokens = 0
    steps_counted = 0
    for i, row in history.iterrows():
        filtered = row.get("batch/filtered_prompts")
        seq_len = row.get("val/sequence_lengths")
        if pd.notna(filtered) and pd.notna(seq_len):
