...
project = 'ai2-llm/olmo3'
# group = "stego32-highlr-filter3"
# name = "mitchish1"
...
for (num_gpus, run) in runs:
    ...
    for i, row in run.history(stream="events").iterrows():
        ...
        if len(power_keys) > 0:
            ...
            if "system.gpu.0.powerWatts" not in power_keys:
                continue
            if "system.gpu.1.powerWatts" not in power_keys:
                continue
            if "system.gpu.2.powerWatts" not in power_keys:
                continue
            if "system.gpu.3.powerWatts" not in power_keys:
                continue
            if "system.gpu.4.powerWatts" not in power_keys:
                continue
            if "system.gpu.5.powerWatts" not in power_keys:
                continue
            if "system.gpu.6.powerWatts" not in power_keys:
                continue
            if "system.gpu.7.powerWatts" not in power_keys:
                continue
