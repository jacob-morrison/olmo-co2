...
for project in projects:
    ...
    for run in runs_raw:
        # if run.group == group:
        if "olmo3-7b-dpo" in run.name.lower() or "olmo3-32b-dpo" in run.name.lower() or "retrojm" in run.name.lower() or "sm0922" in run.name.lower():
            # print(run.settings)
            # print(vars(run))
            # print()
            # print(run.config)
...
#         if get_script_name(run) != 'dpo_tune_cache.py':
#             # print(get_script_name(run))
#             continue
#         else:
#             print(get_script_name(run))
#         if not is_in_date_range(run, START_DATE, END_DATE):
#             print(datetime.fromisoformat(run.created_at.replace('Z', '+00:00')))
#             continue
#         if "rag" in run.name:
#             print(f"rag in name: {run.name}")
#             continue
#         else:
#             print(run.name)
#             # print(vars(run))
...
#     kwh += total_kwh * (gpu_hours / 8)

# # Coreweave, on the east coast
# print()
# print(f'Total trainer gpu hours: {gpu_hours / 3600}')
